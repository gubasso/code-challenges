...
- SM-2 algorithm
- card = problem (code challenge) = dir
- card metadata:
  - toml inside each dir?
  - pure files?
- timer
  - default: 15 min + 5 min delay
integrate with git
  - each interaction is commited
...
# n of days. Initial = 1
interval = 1
# successful recalls (rating >= 3)
n_success = 0
# E-Factor, easiness factor
e_factor = 2.5
# date + interval
scheduled_to = 1979-05-28
...
- start review (sort from deck and pop-up a card)
  - can pop-up one-by-one or the entire session list at once
- start **timer**
  - default 15 min
- do the review
- after: input rating
  - if timer is off, auto rating as 0 (5 min delay)
- program changes:
  - rating = `user_input`
  - date = `system_date_today`
  - e_factor: `(e_factor, rating) => new_e_factor`
  - interval: `(rating, n_success, interval) => new_interval`
  - scheduled_to: `date + interval`
  - n_success: `(rating, n_success) => new_n_sucess`
...
3. **Repetition Number (n)** : This indicates how many times a flashcard has been successfully recalled.
  - "successfully recalled" typically refers to achieving a recall rating of 3 or higher on the 0-5 scale
